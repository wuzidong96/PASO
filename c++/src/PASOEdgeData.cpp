...
{
    double d1 = fuelTimeFunD1(time_min_) + lambda;
    double d2 = fuelTimeFunD1(time_max_) + lambda;
                                  ...
                                  int N)
{
    if(V < 1e-10 || N <= 0)
    {
        std::cout << "quantizeFuelTimeFun input wrong! src=" << src_ << ",des=" << des_ << std::endl;
        exit(-1);
    }
    
    bool verbose = false;
#if 0    
    if(src_ == 4930 && des_ == 4931)
    {
        verbose  = true;
    ...
    double c_max = fuelTimeFun(time_min_);
    double c_min = fuelTimeFun(time_max_);
    // the maximal quanzited level
    int n_max = std::min(static_cast <int> (std::floor(c_max/V))+1, N);
    int n_min = std::min(static_cast <int> (std::floor(c_min/V))+1, N);
    
    // although we do not need to push them according to levels, let us do it here, so we will
    // push n_max finally
    // t_max_ is not necessary the representative time for level n_min, so we should check n_min
    // but t_min_ is for surely the representative time for level n_max, so we do not need to check n_max
    ...
    {    
        if(verbose)
        {
            ...
            std::cout << "fuelTimeFun(time_min_)=" << fuelTimeFun(time_min_) << std::endl;
            std::cout << "fuelTimeFun(time_max_)=" << fuelTimeFun(time_max_) << std::endl;
        }
    ...
    //remember to push back n_max
    quantized_value_.push_back(n_max);
    representative_time_.push_back(time_min_);
    
    //sanity check
    if(quantized_value_.empty() || 
       representative_time_.empty() 
       || quantized_value_.size() != representative_time_.size())
    {
        std::cout << "quantizeFuelTimeFun wrong! src=" << src_ << ",des=" << des_ << std::endl;
        exit(-1);
    }
    return 0;
}
...
PASOEdgeData::updateGphSpeedLimits() 
{
    ...
    while(std::abs(d) > tol && iter <= max_iter)
    {
        ...
        {
            //20170718: there is a bug here. Originally it is t=t, which will make 
            //the speed/time region smaller (a proper subset of true speed/time region).
