...
PASOSolution::updateSpeedDistanceFuelCost(const TPt <TNodeEDatNet<PASONodeData, PASOEdgeData> >& PNet)
{
    for(int i=0; i < time_.size(); i++)
    {
        ...
        int des = path_[i+1];
        double time = time_[i];
        if(time < PNet->GetEDat(src,des).getTimeMin() - 1e-10 
        || time > PNet->GetEDat(src,des).getTimeMax() + 1e-10)
        {
            std::cout << "time is not in range, exit" << std::endl;
            exit(-1);
        }
        PASOEdgeData& edge_data = PNet->GetEDat(src, des);
        distance_[i] = edge_data.getDistance();
        speed_[i] = distance_[i]/time;
        if(speed_[i] < PNet->GetEDat(src,des).getSpeedMin() - 1e-10 
        || speed_[i] > PNet->GetEDat(src,des).getSpeedMax() + 1e-10)
        {
            std::cout << "time is not in range, exit" << std::endl;
            exit(-1); 
        }
