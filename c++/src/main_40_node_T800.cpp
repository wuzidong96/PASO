...
/*======================================== input and output =======================================*/    
    ...
    if(source_id < 1 || source_id > 40 || sink_id < 1 || sink_id > 40)
    {
        cout << "usage: " << argv[0] 
             << " <source id [1-40]> <sink id [1-40]>"
             << endl;
        cout << "source id and sink  id must in [1,40]" << endl;
        return -1;
    }
    cout << "Running for instance (source_id,sink_id)=" << "(" << source_id << "," << sink_id << ","  << ")" << endl;
    
    //several input files
    string node_40_file = "data/40_nodes.txt";
    int source = PASOUtil::getNodeFrom40NodeFile(source_id, node_40_file);
    int sink = PASOUtil::getNodeFrom40NodeFile(sink_id, node_40_file);
    if(source == -1 || sink == -1)
    {
