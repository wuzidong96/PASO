...
coef_file = 'coef_file_gph_T800.txt';
...
%ylim([0,22]);
xlim([10,70]);
h_legend = legend('data (-1%)','fitted (-1%)',...
       'data (0%)','fitted (0%)', ...
       'data (1%)','fitted (1%)','Location','northwest');
...
h_legend = legend('data (-1%)','fitted (-1%)',...
       'data (0%)','fitted (0%)', ...
       'data (1%)','fitted (1%)','Location','northeast');
set(h_legend,'FontSize',14);
box on;
grid on;
%export_fig('fig/power_speed_fit_grade_zero', '-pdf','-transparent','-nocrop');
...
save('coef_fit_gph_T800.mat', 'grade_level_vec', 'coefa', 'coefb', 'coefc', 'coefd');